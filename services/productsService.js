...
const validateProducts = (body) => {
  ...
  const { error } = productSchema.validate({ name, price, ingredients });
...
const newProduct = async(name, price, ingredients) => {
  const productId = await createProduct(name, price, ingredients);
  const product = {
    _id: productId,
    name,
    price,
...
const editProduct = async(id, name, price, ingredients) => {
  await updateProducts(id, name, price, ingredients);
...
const newImage = async(id, host) => {
  ...
  const { _id, name, price, ingredients } = await findProductById(id);
